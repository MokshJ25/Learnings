...
const newLine = (): BillLineItem => ({
  ...
  weight: 0,
  price: 0,
gst_rate: 0, // Default GST rate (N/A)
  gst_amount: 0,
  total: 0,
});
...
const [sgstPercent, setSgstPercent] = useState(0);
  ...
  const [billType, setBillType] = useState<"raw" | "gst">("raw");
...
const cgstAmount = useMemo(() => cgstPercent > 0 ? (subtotal * (cgstPercent / 100)) : 0, [subtotal, cgstPercent]);
const sgstAmount = useMemo(() => sgstPercent > 0 ? (subtotal * (sgstPercent / 100)) : 0, [subtotal, sgstPercent]);
  const finalTotal = Math.max(0, subtotal - discount + cgstAmount + sgstAmount);
  ...
  const updateLine = useCallback((id: string, updates: Partial<BillLineItem>) => {
    setLines((prev) =>
      prev.map((l) => {
        ...
        updated.gst_amount = (updated.total * updated.gst_rate) / 100;
        // Total includes GST
        updated.total = updated.total + updated.gst_amount;
        return updated;
      })
    );
  }, []);
  ...
  const saveBillMutation = useMutation({
    ...
    onSuccess: (bill) => {
      ...
      if (showBillOptions) {
        ...
        if (billType === "raw") {
          generateRawBillPDF(billData);
        } else {
          generateGSTInvoicePDF(billData);
        }
      }
  ...
  // PDF Generation
  const generateRawBillPDF = useCallback((billData: any) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(12);
    doc.text("Main Road, Karahi(Khargone), Madhya Pradesh - 451001", pageWidth / 2, yPosition, { align: "center" });
    yPosition += 10;
    ...
    doc.setFontSize(12);
    doc.text(`Subtotal: ₹${billData.subtotal.toFixed(2)}`, pageWidth - 60, yPosition);
    yPosition += 8;
    ...
    yPosition += 5;
    doc.text("Made by Moksh", pageWidth / 2, yPosition, { align: "center" });

    doc.save(`Bill_${billData.bill_number}_${billData.customer_name.replace(/\s+/g, '_')}.pdf`);
  }, []);

  const generateGSTInvoicePDF = useCallback((billData: any) => {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    let yPosition = 20;
    ...
    // Table Header
    const headers = ["Description of Goods", "GST Rate", "Quantity", "Rate (incl. tax)", "Discount", "Amount"];
    const columnWidths = [50, 20, 20, 30, 25, 30];
    let xPosition = 20;
    ...
    billData.items.forEach((item: any) => {
      ...
      doc.text("₹0.00", xPosition, yPosition); // No discount per item
      xPosition += columnWidths[4];
    ...
    // Amount in Words
    const amountInWords = numberToWords(Math.floor(billData.total)) + " rupees only";
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Amount in Words: ${amountInWords}`, 20, yPosition);
    yPosition += 20;

    // Signature
    doc.text("Authorised Signatory", pageWidth - 60, yPosition);
    yPosition += 15;
                ...
                {line.item_name && (
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input type="number" step="0.01" min="0" max="100" className="h-8 text-sm" value={line.gst_rate} onChange={(e) => updateLine(line.id, { gst_rate: parseFloat(e.target.value) || 0 })} />
                    </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" step="0.01" min="0" max="100" value={cgstPercent} onChange={(e) => setCgstPercent(parseFloat(e.target.value) || 0)} placeholder="9.00" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm">SGST %</Label>
                <Input type="number" step="0.01" min="0" max="100" value={sgstPercent} onChange={(e) => setSgstPercent(parseFloat(e.target.value) || 0)} placeholder="9.00" />
              </div>
            </div>
            ...
            <div className="space-y-3 border-t border-border pt-3">
              <div className="flex items-center gap-2">
                <Checkbox id="printBill" checked={showBillOptions} onCheckedChange={(c) => setShowBillOptions(!!c)} />
                <Label htmlFor="printBill" className="text-sm cursor-pointer">Generate PDF Bill</Label>
              </div>
              ...
              {showBillOptions && (
                <div className="pl-6 space-y-3 animate-fade-in">
                  <div className="space-y-2">
                    ...
                    <Select value={billType} onValueChange={(v: "raw" | "gst") => setBillType(v)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="raw">Raw Bill</SelectItem>
